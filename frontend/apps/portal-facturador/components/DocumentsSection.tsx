...
export default function DocumentsSection({ token }: Props) {
  ...
  const handleDownload = async (docId: string, format: "pdf" | "xml") => {
    try {
      const res = await fetch(`${apiUrl}/invoicing/documents/${docId}/${format}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
      if (!res.ok) {
        const err = await res.json().catch(() => ({}));
        setActionMsg(err.detail || `Error ${res.status} al descargar ${format.toUpperCase()}`);
      ...
      setActionMsg(`Error de conexion al descargar ${format.toUpperCase()}`);
      setTimeout(() => setActionMsg(""), 4000);
    }
  };
              ...
              <tr key={doc.id} className="border-b border-white/5 hover:bg-white/5 transition">
                ...
                <td className="px-4 py-3">
                  <div className="flex items-center justify-center gap-1">
                    <button onClick={() => handleDownload(doc.id, "pdf")} title="Descargar PDF"
                    ...
                    <button onClick={() => handleDownload(doc.id, "xml")} title="Descargar XML"
